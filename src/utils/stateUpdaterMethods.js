...
export const stateMethods = {
  ...
  distToPlanets: (planetDistanceData) => {
    const pDistance = {};
  ...
  setSelectors: (Selectors, vehSelection, commonState) => {
    commonState.setState ({
      [Selectors]: stateMethods.setVehicle(commonState.state.vehicles),
      [vehSelection]: '',
    });
  },
		...
		return vehicles.map(vehicleObj => {
			...
			if(vehicleObj.name === vehSelection1){
				...
				if(vehicleObj.totalNumber === -1){
					...
					stateMethods.setSelectors('destination1Vehicles', 'vehSelection1', commonState);
				}
			}
			if(vehicleObj.name === vehSelection2){
				...
				if(vehicleObj.totalNumber === -1){
					vehicleObj.totalNumber = 0;
					stateMethods.setSelectors('destination2Vehicles', 'vehSelection2', commonState);
				}
			}
			if(vehicleObj.name === vehSelection3){
				...
				if(vehicleObj.totalNumber === -1){
					vehicleObj.totalNumber = 0;
					stateMethods.setSelectors('destination3Vehicles', 'vehSelection3', commonState);
				}
			}
			if(vehicleObj.name === vehSelection4){
				...
				if(vehicleObj.totalNumber === -1){
					vehicleObj.totalNumber = 0;
					stateMethods.setSelectors('destination4Vehicles', 'vehSelection4', commonState);
				}
			}
			return vehicleObj
		});
  ...
  getDestinationAndVehiclesJson: async (commonState) => {
        ...
        commonState.setState({
            planets: planetData,
            planetDistance: stateMethods.distToPlanets(planetData),
    })
		...
		const vehicleData = await getter('vehicles')
				...
				commonState.setState ({
					...
					vehiclesSpeed: stateMethods.vehSpeeds(vehicleData),
				});
